...
最终命令将这三步串联，并重定向输出到 cleaned_data.csv。`,
      verify: `处理完成后，我首先使用 \`wc -l\` 命令统计了清洗前后的行数。原始数据为 ${10000 + c.personalizedData.addedRecords} 行，清洗后为 ${10000 + c.personalizedData.addedRecords - 18} 行，说明有 18 条异常数据被成功剔除。
接着，我使用 \`head -n 5\` 查看了清洗后的前 5 行，确认 Type 字段已变更为 Full Name，且无空字段。
最后，将清洗干净的数据上传至 HDFS 的 \`/user/${c.terminalConfig.user}/clean/\` 目录，准备进行下一步存储。`
    },
    2: { // 李华 - 简洁
...
执行命令：\`cat industrial_data.csv | awk -F, '$6>0' | sed 's/product/PRODUCT/g' > processed_data.csv\`。`,
      verify: `验证手段：
1. 计数验证：\`wc -l\` 显示有效数据剩余 ${10000 + c.personalizedData.addedRecords - 15} 条。
...
- Input Rows: ${10000 + c.personalizedData.addedRecords}
- Output Rows: ${10000 + c.personalizedData.addedRecords - 25} (Dropped 25 outliers)
